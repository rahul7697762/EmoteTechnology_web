      ...
      <main className="flex-grow container mx-auto px-6 py-12 lg:py-20 relative z-10 max-w-7xl">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-12 mb-32">
          <motion.div variants={containerVars} initial="hidden" animate="show" className="flex-1 max-w-3xl">
            ...
            <motion.p variants={itemVars} className="text-lg md:text-xl text-[var(--dim)] leading-relaxed mb-6">
            ...
            <motion.p variants={itemVars} className="text-lg text-[var(--dim2)] leading-relaxed max-w-2xl">
        ...
        <motion.div 
          ...
          className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-32 border-y border-[var(--border-glass)] py-12"
        >
          {[
            ['2019', 'founded'],
            ...
            <div key={i} className={cn("flex flex-col items-center justify-center text-center", i !== 0 && "md:border-l md:border-[var(--border-glass)]")}>
              <div className="text-5xl lg:text-7xl font-bold text-[var(--text-main)] mb-2 drop-shadow-md">{n}</div>
        ...
        <div className="mb-32">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {team.map((member, i) => (
              <div key={i} className="p-6 rounded-[2rem] border border-[var(--border-glass)] bg-[var(--bg-glass)] hover:bg-[var(--bg-glass-heavy)] transition-colors flex flex-col items-center text-center group shadow-lg">
                <div className="w-20 h-20 rounded-full bg-[var(--panel-bg)] border border-[var(--border-glass)] flex items-center justify-center mb-6 shadow-inner group-hover:border-blue-500/30 transition-colors text-3xl font-bold text-blue-400">
                  {member.name[0]}
                </div>
                <h3 className="text-xl font-bold text-[var(--text-main)] mb-2">{member.name}</h3>
        ...
        <div className="mb-32 grid lg:grid-cols-2 gap-16 items-start">
          <div className="sticky top-32">
            ...
            <p className="text-lg text-[var(--dim)]">Our manifesto for shipping great software.</p>
          </div>
          <div className="flex flex-col gap-6">
            {rules.map((rule, i) => (
              <div key={i} className="p-8 rounded-3xl border border-[var(--border-glass)] bg-gradient-to-r from-[var(--bg-glass)] to-transparent hover:from-[var(--bg-glass-heavy)] transition-colors flex gap-6 items-center group shadow-lg">
                <span className="text-5xl font-bold text-[var(--border-glass)] group-hover:text-blue-500/20 transition-colors font-mono">0{i + 1}</span>
                <p className="text-xl font-medium text-[var(--text-main)] transition-colors">{rule}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Office Location */}
        <div className="mb-20">
          <div className="p-8 md:p-12 rounded-[2.5rem] border border-blue-500/20 bg-gradient-to-br from-blue-900/10 to-[var(--panel-bg)] backdrop-blur-xl shadow-2xl flex flex-col md:flex-row items-center justify-between gap-12">
            <div>
              ...
              <p className="text-xl text-[var(--dim)] mb-8">(And online everywhere else.)</p>
            ...
            <div className="relative w-48 h-48 md:w-80 md:h-80 rounded-full border border-[var(--border-glass)] bg-[var(--bg-glass)] flex items-center justify-center overflow-hidden shrink-0">
              ...
              <div className="absolute w-full h-[1px] bg-[var(--border-glass)]" />
              <div className="absolute h-full w-[1px] bg-[var(--border-glass)]" />
        ...
        {/* CTA */}
        <div className="text-center py-20 border-t border-[var(--border-glass)]">
          <h2 className="text-5xl md:text-7xl font-bold mb-6 tracking-tight">Hiring? <span className="text-[var(--dim2)]">No.</span><br />Always reading? <span className="text-blue-400">Yes.</span></h2>
          <p className="text-xl text-[var(--dim)] max-w-2xl mx-auto mb-10">Send a paragraph + a thing you've made. We reply to everyone.</p>
          <Link href="mailto:hr@emotetechnology.in" className="inline-flex items-center gap-3 bg-[var(--text-main)] text-[var(--page-bg)] px-8 py-4 rounded-xl font-bold overflow-hidden hover:scale-[1.02] transition-transform shadow-lg">
